...
import { useRemoveBarangMutation } from '@/hooks/api/barangSliceAPI';

export default function FormHapusBarang({ initialData, onCancel, onSuccess }) {
    const [removeBarang, { isLoading, isError, error }] = useRemoveBarangMutation();
    const [namaBarang, setnamaBarang] = useState('');

    const isMatch = namaBarang.trim() === initialData?.namaBarang;
    ...
    const handleRemove = async (e) => {
        ...
        try {
            const result = await removeBarang({
                id: initialData.id,
                data: { namaBarang },
            }).unwrap();
            ...
            <div className="px-6 py-5 space-y-4">
                ...
                <div>
                    <label className="block text-xs font-semibold text-gray-500 mb-1.5">
                        Ketik <span className="text-gray-900">{initialData?.namaBarang}</span> untuk konfirmasi
                    </label>
                    <input
                        type="text"
                        value={namaBarang}
                        onChange={(e) => setnamaBarang(e.target.value)}
                        placeholder={initialData?.namaBarang}
                        autoFocus
