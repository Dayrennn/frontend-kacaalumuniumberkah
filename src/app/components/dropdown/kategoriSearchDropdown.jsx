...
export default function KategoriSearchDropdown({ value, onChange, initialLabel }) {
    const [search, setSearch] = useState(initialLabel || value || '');
    const [open, setOpen] = useState(false);
    const wrapperRef = useRef();

    const { data: response, isLoading } = useSeeAllKategoriQuery();
    const kategoriList = response?.data?.kategori ?? [];

    const filtered = kategoriList.filter((k) => k.namaKategori.toLowerCase().includes(search.toLowerCase()));
    ...
    const handleSelect = (kategori) => {
        setSearch(kategori.namaKategori);
        onChange(kategori.id);
        setOpen(false);
    };
